...
# Class: emmserver
#
# This class installs WSO2 EMM Server
#
# Parameters:
# version            => '5.2.1'
# offset             => 1,
...
#      'conf/user-mgt.xml',
  ...
  emmserver::clean { $deployment_code:
    mode   => $maintenance_mode,
    target => $carbon_home,
  }

  emmserver::initialize { $deployment_code:
    repo      => $package_repo,
  ...
  emmserver::deploy { $deployment_code:
    security => true,
    owner    => $owner,
    group    => $group,
    target   => $carbon_home,
    require  => Emmserver::Initialize[$deployment_code],
  }

  emmserver::push_templates {
    $service_templates:
      target    => $carbon_home,
      directory => $deployment_code,
      require   => Emmserver::Deploy[$deployment_code];
  }
